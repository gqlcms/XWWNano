...
class fetchFiles():
    ...
    def cmsconnetFile(self,ifile):
        ...
        wget = "wget --tries=3 --no-check-certificate http://stash.osgconnect.net/+qilongguo/{file} -O {PATH}/In_{filename}"
        ifile = ifile.replace("/stash/user/qilongguo/public/","")
    ...
    def GetFile(self,isNanoAOD):
        ...
        for ifile in self.files:
            if "/stash/user/qilongguo/public" in ifile:
