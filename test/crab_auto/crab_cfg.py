...
def main():
  ...
  parser.add_option('-y', dest='year', help='which year sample', default='2018', type='string')
  parser.add_option('-m', dest='mode', default='local', help='runmode local/condor')
  #parser.add_option('-i', '--in', dest='inputs', help='input directory with files', default=None, type='string')
  parser.add_option('-d', dest='isdata', help='to apply sf correction or not', action='store_true')
  parser.add_option('-p', dest='period',default="B", help="Run period, only work for data")
  parser.add_option('-f', dest='file', default='', help='File input. In local mode it will be the filepath. In condor mode it will be the dataset name')

  #parser.add_option('-o', '--out', dest='output', help='output directory with files', default=None, type='string')
  (opt, args) = parser.parse_args()

  print 'isdata:',opt.isdata
