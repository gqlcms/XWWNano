...
class fetchFiles():
    ...
    def cmsconnetFile(self,ifile):
        ...
        User_ = re.compile(r'/stash/user/(.*)/public/')
        file_ = re.compile(r'/stash/user/(.*)/public/(.*)')
        User = User_.search(ifile).group(1)
        File = file_.search(ifile).group(2)
        wget = "wget --tries=3 --no-check-certificate http://stash.osgconnect.net/+{User}/{file} -O {PATH}/In_{filename}"
    ...
    def GetFile(self,isNanoAOD):
        ...
        for ifile in self.files:
            if ifile.startswith("/stash/user/") :
