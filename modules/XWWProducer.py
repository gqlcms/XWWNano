...
class VVVProducer(Module):
    ...
    def Selection_ggF(self,event):
        ...
        Mj_max  = 0 ; PTj_max = 0 
        for inum in range(0,event.nFatJet):
            if( event.FatJet_pt[inum] > 200 and abs(event.FatJet_eta[inum])<2.4 and inum<5 and (event.FatJet_jetId[inum]&2>0) ) :
                fatjet = TLorentzVector()
                fatjet.SetPtEtaPhiM(event.FatJet_pt[inum],event.FatJet_eta[inum],event.FatJet_phi[inum],event.FatJet_msoftdrop[inum])
                FatJets.append(fatjet)
                if fatjet.M()  > Mj_max  : Mj_max  = fatjet.M()
                if fatjet.Pt() > PTj_max : PTj_max = fatjet.Pt()
    ...
    def analyze(self, event):
        ...
        PassVBF = self.Selection_VBF(event)
        PassggF = self.Selection_ggF(event)
        ...
        if PassVBF : VBF = 1
        if PassggF : ggF = 1
        self.out.fillBranch("PassggF", ggF)
