...
def OptimalCut(B,S):
    Bins = B.GetNbinsX(); B0=B.Integral(); S0=S.Integral(); SigMax=0; CutLocation_L=0; CutLocation_R=0; BKGRejection=0; Sig_Eff=0;
...
class ANALYSIS:
    ...
    def construct_plot(self,Nj,variable,cut,cut1,tag,nbin,min,max,xtitle="",ytitle="",logy=1,CR=0):
        ...
        if MODE in ["MCvsDATA"]:
            t_data  = TChain("NewTree");t_data.Add(path+"JetHT/18.root");
        ...
        t_Signal2 = TChain("NewTree"); t_Signal2.Add(path + "Signal/HWW4q_PS.root"); h_Signal2 = TH1D("h_Signal2", "h_Signal2" + ";%s;%s" % (xtitle, ytitle), nbin, min, max); h_Signal2.Sumw2();
        ...
        t_QCD     = TChain("NewTree");  t_QCD.Add(    path+"BKG/QCD.root"  );  h_QCD    =TH1D("h_QCD"    ,"h_QCD"    +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_QCD.Sumw2();   
        t_TTbar   = TChain("NewTree");  t_TTbar.Add(  path+"BKG/TT.root"   );  h_TTbar  =TH1D("h_TTbar"  ,"h_TTbar"  +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_TTbar.Sumw2(); 
        t_STop    = TChain("NewTree");  t_STop.Add(   path+"BKG/ST.root"   );  h_STop   =TH1D("h_STop"   ,"h_STop"   +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_STop.Sumw2();  
        t_WJets   = TChain("NewTree");  t_WJets.Add(  path+"BKG/WJ.root");  h_WJets  =TH1D("h_WJets"  ,"h_WJets"  +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_WJets.Sumw2(); 
        t_Rest    = TChain("NewTree");  t_Rest.Add(   path+"BKG/Rest.root" );  h_Rest   =TH1D("h_Rest"   ,"h_Rest"   +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_Rest.Sumw2();   

        if MODE in ["DECO"]:
            for c in ["R4q","R3q","R2q","W","Rlqq","Rlq","g",   "Rest"]:
        ...
        if MODE in ["DECO"]:            #comp = ["R4q","R3q","R2q","W","Rlqq","Rlq","Rest"];
            ...
            for h in [h_TotalS1, h_Signal1_R4q, h_Signal1_R3q, h_Signal1_R2q, h_Signal1_W, h_Signal1_Rlqq, h_Signal1_Rlq, h_Signal1_g,   h_Signal1_Rest]: h=UnderOverFlow1D(h);
        ...
        if MODE in ["DECO"]:
            ...
            h_TotalMC.Scale( h_Signal1.Integral()/h_TotalMC.Integral() );
        ...
        if MODE in ["DECO"]:
            ...
            theLeg.AddEntry(h_Signal1,"(2500, 250) GeV","L");
            ...
            theLeg.AddEntry(h_Signal1_R4q ,"R^{4q}  %.0f"%(  h_Signal1_R4q.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
            theLeg.AddEntry(h_Signal1_R3q ,"R^{3q}  %.0f"%(  h_Signal1_R3q.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
            theLeg.AddEntry(h_Signal1_R2q ,"R^{2q}  %.0f"%(  h_Signal1_R2q.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
            theLeg.AddEntry(h_Signal1_W   ,"W       %.0f"%(    h_Signal1_W.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
            theLeg.AddEntry(h_Signal1_Rlqq,"R^{lqq} %.0f"%( h_Signal1_Rlqq.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
            theLeg.AddEntry(h_Signal1_Rlq ,"R^{lq}  %.0f"%(  h_Signal1_Rlq.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
            theLeg.AddEntry(h_Signal1_g   ,"g       %.0f"%(    h_Signal1_g.Integral()/h_TotalS1.Integral()*100)+"%" ,"L");
